...
public class AddonAction {
  ...
  public AddonAction(TestCaseStepEntity testCaseStepEntity, TestCaseStepResult result,
                     LinkedList<ElementPropertiesEntity> addonElementPropertiesEntity, Map<String, String> envSettings)
  ...
  throws NoSuchMethodException, IllegalAccessException, InvocationTargetException, InstantiationException, IOException, ClassNotFoundException {
    ...
    this.addonElementPropertiesEntity = addonElementPropertiesEntity;
  ...
  private void loadClasses() throws IOException, ClassNotFoundException, NoSuchMethodException, IllegalAccessException, InvocationTargetException, InstantiationException {
    ...
    this.clazz = addonService.loadJarClass(jarFilePath, entity.getFullyQualifiedName(), true);
  ...
  private void afterExecute() throws NoSuchFieldException, IllegalAccessException, AutomatorException {
    saveRunTimeData();
    setAddonLogsInTestStep(instance);
  }
  ...
  private void setAddonLogsInTestStep(Object instance) throws NoSuchFieldException, IllegalAccessException {
    log.info("Saving Addon Logs in Test Step Result");
    ...
    result.setAddonActionLogs(val.toString());
  ...
  public void setElements() throws AutomatorException, NoSuchMethodException, InstantiationException, IllegalAccessException, InvocationTargetException {
    ...
    if (!elementsMap.isEmpty()) {
      for (AddonNaturalTextActionParameterEntity addonNaturalTextActionParameterEntity : addonNaturalTextActionEntity.getPluginParameters()) {
        if (addonNaturalTextActionParameterEntity.getType() == AddonActionParameterType.ELEMENT) {
          log.info("Setting Element for Addon Plugin Action Step - " + addonNaturalTextActionParameterEntity);
          ...
          Object elementInstance = addonService.getElementInstance(elementPropertiesEntity.getLocatorValue(),
  ...
  public void setTestData() throws NoSuchMethodException, InstantiationException, IllegalAccessException, InvocationTargetException {
    ...
    if (!testDataMap.isEmpty()) {
      for (AddonNaturalTextActionParameterEntity addonNaturalTextActionParameterEntity : addonNaturalTextActionEntity.getPluginParameters()) {
        if (addonNaturalTextActionParameterEntity.getType() == AddonActionParameterType.TEST_DATA) {
          log.info("Setting Test Data for Addon Plugin Action Step - " + addonNaturalTextActionParameterEntity);
          Object testDataInstance = addonService.getTestDataInstance(testDataMap.get(addonNaturalTextActionParameterEntity
