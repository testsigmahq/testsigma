...
@Component({
  selector: 'app-zepel-issue-form',
  templateUrl: './zepel-issue-form.component.html',
  styles: []
})
export class ZepelIssueFormComponent extends BaseComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.applicationService.getZepelProjects(this.application.id).subscribe(res => {
      this.projects = res['squads'];
  ...
  fetchIssueTypes() {
    this.applicationService.getZepelIssueTypes(this.application.id, this.selectedProject.id).subscribe(res => {
      this.issueTypes = res['lists'];
  ...
  fetchIssues(term?) {
    this.applicationService.searchZepelIssues(this.application.id, this.selectedProject.id, this.selectedIssueType.id).subscribe(res => {
      this.isFetchingIssues = false;
      this.issueList = term ? res['items'].filter(issue => (issue.title.includes(term))) : res['items'];
    });
  }
