...
public class GetTextBoxesWithLabelAction extends SuggestionAction {
  @Override
  protected void execute() throws Exception {
    List<WebElement> textLabels = getDriver().findElements(By.xpath("//input[@type='text']//preceding::label[1]"));
    Assert.isTrue(textLabels.size() != 0, String.valueOf(SuggestionActionResult.Failure));
    List<Map<String, String>> list = new ArrayList<Map<String, String>>();
    for (WebElement element : textLabels) {
      WebElement textBox = getDriver()
        .findElement(By.xpath("//label[text()='" + element.getText() + "']//following::input[@type='text'][1]"));
