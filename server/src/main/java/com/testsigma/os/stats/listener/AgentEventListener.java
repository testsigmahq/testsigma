  ...
  public void OnAgentEvent(AgentEvent<Agent> event) {
    ...
    try {
      if (event.getEventType() == EventType.CREATE) {
        testsigmaOsStatsService.sendAgentStats(event.getEventData(), com.testsigma.os.stats.event.EventType.CREATE);
      } else if (event.getEventType() == EventType.DELETE) {
        testsigmaOsStatsService.sendAgentStats(event.getEventData(), com.testsigma.os.stats.event.EventType.DELETE);
      }
